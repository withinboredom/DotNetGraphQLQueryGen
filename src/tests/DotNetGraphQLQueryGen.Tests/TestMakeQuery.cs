...
namespace CoreData.Model.Tests
{
    ...
    public class TestTypedQuery
    {
        ...
        public void SimpleArgs()
        {
            ...
            var query = client.MakeQuery(q => new {Movies = q.Movies(s => new {s.Id})});
            Assert.Equal(
                @"query BaseGraphQLClient {
Movies: movies {
Id: id
}
}".Replace("\r\n", "\n"),
        ...
        {
            var client = new TestClient();
            var query = client.MakeQuery(q => new {Actors = q.Actors(s => new {s.Id, DirectorOf = s.DirectorOf()})});
            Assert.Equal(
                @"query BaseGraphQLClient {
Actors: actors {
Id: id
            ...
            var query = client.MakeMutation(
                q => new
                {
                    Movie = q.AddMovie(
                        ...
                        null,
                        null,
                        new DateTime(2019, 10, 30, 17, 55, 23),
                        s => new {s.Id})
                },
                true);
            Assert.Equal(
                @"mutation BaseGraphQLClient {
...
Id: id
        ...
        {
            var client = new TestClient();
            var query = client.MakeQuery(q => new MyResult {Movies = q.Movies(s => new MovieResult {Id = s.Id})});
            Assert.Equal(
                @"query BaseGraphQLClient {
Movies: movies {
Id: id
